...
def test_default_registry_order():
    """注册表顺序 = 正式插件优先 (更保守者前) + 旧 classify 探测顺序.

    轮 2 起默认注册: arc_curvature (插件 3, 开链/短弧, 保守) →
    ellipse_group_label (插件 1, 闭合整椭圆) → line → circle →
    ellipse → general (优先级规则见 docs/boundary_plugins.md).
    插件判定优先, 未命中让位内置."""
