...
def test_d11_counts_current():
    """计数回填真实值: 30 顶层模块 / 133 测试文件 / ~1546 函数 (9.28.0 实测)."""
    text = _text()
    assert "30 顶层模块" in text, "顶层模块计数未回填"
    assert "133 文件" in text, "测试文件计数未回填"
    assert "1546" in text, "test 函数计数未回填 (grep def test_ 计数)"
    assert "~1546 函数" in text, "测试体系节标题计数未回填"
