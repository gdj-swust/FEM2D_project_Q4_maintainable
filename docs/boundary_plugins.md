...
## 已知悬崖: 粗网格角弧识别断裂 (C2 确认项, 2026-08-06 — 待总指挥决策)

> 审查发现 (review_20260805 R5, 已复现): 圆角矩形角弧每侧 ≤4 条网格边时,
> 角弧整段丢失为直线, 识别结果随网格密度突变 (14→8 段, `a` 快捷选择
> 粗网格落空); 每侧 ≥5 条边时 4 line + 4 arc 正常。

**机制** (两个保守门叠加, 均为有意设计):
1. `sharp_corner_indices` 20° 角点门 (topology.py `_segment_loop`) —
   稀疏多边形必须保留可见角 (geometry.py `_closed_conic_turn_limit_deg`
   注释: "A sparse polygon must retain its visible corners")。
   粗网格角弧 (每侧 3-4 条边) 相邻边转向 ~22°-30° > 20° → 弧被切成
   2 节点小段。
2. CircleDetector 开放弧 ≥4 点门 (detectors/circle.py `_open_arc`) —
   3 点必共圆, 拟合残差恒 0, 无判别力; 宁缺毋滥原则 (插件目录:
   "标签冲突以更保守者胜")。

**叠加结果**: 2 节点小段必被判 line (两点必共线) → 角弧以多条短直边
呈现; 段数/标签/`a` 快捷选择随细化突变。C1 修复 (2026-08-06, 共线判据)
后, 这些短直边不再被错误合并成弯曲"直边" — 识别更忠实, 但弧能力
仍受 ≥4 点门限制, 悬崖依旧存在。

**决策备忘** (本包不直接改算法, 方案供总指挥裁决):
- **方案 A (接受现状, 推荐)**: 悬崖是"宁缺毋滥"的已知权衡 — 误标弧
  (把折线/锯齿当弧) 的后果比漏标弧 (弧当多段直边) 更严重。粗网格下
  弧以多段直边呈现, 语义正确 (边界点集不变, 载荷链不受影响 — 只影响
  段标签与 `a` 快捷选择)。配套: 用户文档提示粗网格下 `a` 选择角弧
  可能落空, 建议加密或按边选择。
- **方案 B (协调两门)**: 当角点两侧链均为"短弧候选"时放宽角点门 —
  需全局上下文 (角点门是局部几何, 弧检测是链级), 改动识别结果须走
  金标准特殊程序 (diff 逐条审查 + 快照单独提交), 成本高, 收益低
  (粗网格角弧的物理意义仍只是近似)。
- **方案 C (弧门降到 ≥3 点 + 跨度门)**: 3 点必共圆无判别力, 需额外
  跨度/残差门, 与"宁缺毋滥"原则冲突, 不推荐。

**结论建议**: 接受现状 + 本段文档标注 (方案 A)。若总指挥裁定修改,
须经 C1 同款特殊程序 (金标准复验 + diff 逐条审查) 后另行提交。
...
- ⚠️ **@段名 与 @组名同名时段标签优先**: 自动标签 (如 "直边"/"圆弧") 若与
  用户物理组名同文本且覆盖不同段, `@直边` 会选中自动标签的段而非整个物理
  组 (段标签精确命中先于物理组展开, 见 bc_apply._resolve_boundary_selection)。
  低概率 (自动标签为通用词), 同名时用段编号区分。
