...
| 最低版实测 | numpy 2.0.x | 本机 (numpy 2.5.1) 676 收集 0 失败; 3.13 上最老 2.x (2.1.0) 全量实测同样 0 失败; 3.11×numpy==2.0 CI 组合为声明地板 — numpy 2.0 最高支持 Python 3.12 (3.13 需 numpy>=2.1), 地板组合因此放 3.11 |
...
无 gmsh 时的 skip 行为 (已有守卫, 本文件第 3 节逐一列明; 2026-08-03 全新 venv
实测, 与 runner 镜像无关): **655 collected → 640 passed + 19 skipped, 0 失败**。
...
预期: **676 collected → 674 passed + 2 skipped**。2 个 skip 恒定为
...
无 gmsh 时实测的 19 个 skip 明细 (2026-08-03 venv 实测, `-rs` 输出):

| 文件 | 数量 | 守卫方式 |
|------|------|---------|
| tests/test_boundary_gmsh.py | 8 | 模块级 `try: import gmsh` → 捕 `(ImportError, OSError)` → `pytest.skip(allow_module_level=True)` (OSError 是缺 libGLU 时的真实异常, importorskip 捕不到); pytest 汇总按 1 个模块事件计 |
| tests/test_boundary_complex.py | 5 | 同上 |
| tests/test_boundary_highpressure.py | 5 | 同上 |
...
python -m pytest                             # 有 gmsh: 676 collected → 674 passed + 2 skipped
python -m pip uninstall -y gmsh && python -m pytest   # 无 gmsh: 655 collected → 640 passed + 19 skipped
...
2026-08-03 实测 **674 passed + 2 skipped** — 与最新栈 (2.5.1) 逐项一致, 0 失败。
