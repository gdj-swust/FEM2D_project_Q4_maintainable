...
## 9.31.0 (2026-08-19) — 预处理向量化 + 网格密度一致性 + 变形面板逆映射
...
> 1331–1364) §2.3, 边界节点恢复改由最近内部节点 patch 拟合、在边界节点
> 坐标处求值 — 消除薄边界 patch 单侧外推的 ~O(h) 边界恢复误差。
> 确定性算法, 不改公共 API; 内部节点恢复逐位不变 (P-ε 等价全绿)。

- **fem2d/spr.py**: 新增 `_boundary_patch_table` (ZZ92 §2.3 边界 patch
  增强表): 边界节点集 = boundary_edges 出现过的节点; 候选内部节点 =
  邻接单元顶点 − 边界集, 为空扩 ring-1 (CST 角点情形); 取最近且 patch
  非空者 (并列取最小节点号), 候选仍空 (全边界退化网格) 退回自身 patch
  兜底。`_node_patch_csr` / `_fit_nodes_exact` 加可选 ``table`` 参数
  (默认原表, 逻辑不动); `spr_recovery` 入口构造增强表下传。`_fit_node_block`
  拟合核心零改动 — 内部节点恢复值与改前逐位一致。
- **实测效果** (悬臂梁/Kirsch 基准, docs/spr_boundary_data/):
  - C1 顶边中点 σy: CPS3 0.0081→0.00017 (47×), CPS4 0.0051→0.0026 (2×);
  - C2 右端面 τxy 最大误差: CPS3 0.062→0.013 (4.7×), CPS4 0.052→0.026 (2×);
  - C3 Kirsch 孔边 σθθ: θ=60° 0.588→0.080 (7.4×), θ=90° 0.266→0.120 (2.2×),
    θ=30° 0.075→0.055 (1.4×); θ=0° 0.016→0.123 退化 — 最近内部 patch
    (半径 0.42a) 线性外推平滑 1/r² 曲率, 如实记录 (见验收报告);
  - D 收敛阶 CPS3: syy_top 0.96→1.79, txy_top 0.82→1.79,
    txy_right_q 1.55→1.81, sxx_top 0.19→1.79 (全 O(h)→O(h²));
    CPS4: sxx_top 2.53→2.36, txy_right_q 1.95→1.94 (保持 O(h²));
  - E 效应指数 CPS3 1.55→[0.85, 0.94, 0.98], CPS4 1.00→[0.95, 0.98, 0.99]。
  - CPS4 σy/τxy 顶边点仍 ~O(h): 探针实测 Q4 2×2 Gauss 的 σy 采样误差
    本身 O(h) (νP(L−x)y²/(2EI) 项的 y² 形函数插值残差, 逐行 ±0.289h
    锯齿场), 线性恢复不可消除 — FE 采样一致性下限, 非算法缺陷 (详见
    验收报告 docs/spr_boundary_acceptance_report.md)。
- **测试**: tests/test_spr_boundary.py 新增 (确定性算法单测、边界线性
  精确、C1/C2/C3 阈值、D 收敛阶、E 效应指数、内部不变性);
  P-ε 等价与 naive-LS 参考同步边界替换管线 (SPR-BC-2026-001 重基线)。
