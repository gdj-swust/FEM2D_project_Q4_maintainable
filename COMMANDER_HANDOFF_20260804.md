...
| **@组名批量选择（边载荷/约束）** | **待办（与上条同批，纯输入层增加）**：CLI `--traction "@椭圆孔:0,-2e6"` + 段列表打印处提示"输入编号 12,13 精细选择；输入 @组名 整组选择"。底层已备好：`region_registry.by_name(name, dimension=1)` 返回组内全部曲线（regions.py:195）。**保留编号通道**（20 段独立 = 可对孔局部施力的优点，用户认可）。行为约束：@组名展开结果 == 手输全部编号（判别性测试）；识别/拓扑/施加逻辑零改动；旧编号语法完全兼容 |
| **边界识别审查结论（2026-08-04 会话）** | 无硬伤（静默错结果类已全部验证门化）。两个半硬伤：①段编号跨网格静默错位（segment_sort_key = 环+段长排序，网格加密编号全变，固定脚本用编号施力会静默施加错边——@组名是其解法）②极端拓扑（自交环/嵌套孔）无判别性测试，属测试盲区。物理曲线"椭圆孔"名 = 建模者命名非识别（打印照印组名） |
