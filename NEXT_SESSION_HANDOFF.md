...
> **验收第一件事**: 跑 `python -m pytest` (应 **496 passed**) → 跑 `python run.py models/test_spec.txt --no-plot` 冒烟
...
- **性质**: 2D 有限元教学求解器 (CST/Q4/Q4R/Q4I)，~5.5 万行 Python，无 git
- **测试**: `python -m pytest` = **496 passed**（本会话 374 → 496）；Ruff/Mypy 全干净
